...
void loop(){
  ...
  if(isDayTime()){
    ...
    */
    if((time-currentTime) >= minute){
      currentTime = time;//reset time
    }else{
      if((time-currentTime) < forty){
        if(birdDetected_new()){
      ...
      }else if((time-currentTime) >=forty){
        Serial.println("Time more than 40");
        unsigned long totalDurationPlayed = 0;
        unsigned const long twentySecondsInMS = 20000;
        while(totalDurationPlayed < twentySecondsInMS ){
          playSound();
          totalDurationPlayed = totalDurationPlayed + (unsigned long)soundDuration;
          Serial.println(totalDurationPlayed);
